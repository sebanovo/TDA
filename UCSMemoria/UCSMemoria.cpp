...
{
    if (cadena.length() < 3)
        throw new Exception("La cadena tiene que tener minimo 3 caracteres");
    if (cadena.substr(0, 2) != "->")
        throw new Exception("La cadena necesita una flecha (->)");
    return cadena.substr(2, cadena.length());
}

void CSMemoria::dibujar_celda(TForm* Form, TColor brushColor, bool withBorder,
...
void CSMemoria::dibujar_memoria(TForm* Form, int posX, int posY, int a, int b)
{
    // pending
    if (a > b) {
    ...
    for (int i = a; i <= b; i++) {
        if (mem[i].link != libre) {
            ...
            // Id columna
            dibujar_celda(Form, clYellow, true, posX, posY, mem[i].id.c_str());
            posX += TamanoCeldaX;
...
// ejercicio
// suma + 5 al campo indicado
void CSMemoria::sumar(int dir, string campo)
{
    poner_dato(dir, campo, obtener_dato(dir, campo) + 5);
}
